...
def test_create_delete_pvcs(multi_pvc_factory, pod_factory,project=None):
    ...
    log.info(
        "PVC for pods are Bound. Creating pods for the purpose of "
        "deletion later"
    )
    ...
    log.info("Initial set of rbd and cephfs pvcs and pods are created.")
    input(
        "Press Enter to start creation and deletion parallel process"
        "(The actual test)"
    )
