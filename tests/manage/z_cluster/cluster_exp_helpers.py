...
def raw_block_io(raw_blk_pod, size='10G'):
    ...
    Returns:
        bool : True if IOs happen successfully
...
def downloader(dest_pod, namespace):
    """
     Downloads a wellknown file from the net on a given pod's /mnt directory

    Args:
        dest_pod (pod object) : pod on which the file need to be downloaded
    ...
    def cluster_filler(self):
        ...
        with ThreadPoolExecutor() as executor:
            for pod in self.pods_to_fill:
                executor.submit(downloader, pod, self.namespace)
